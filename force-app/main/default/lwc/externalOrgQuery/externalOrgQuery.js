...
export default class ExternalOrgQuery extends LightningElement {
  ...
  get customObjects() {
    return this.availableObjects.filter(
      (obj) => !this.standardObjects.includes(obj)
    );
  }
  ...
  get isStartImportDisabled() {
    ...
    return (
      !this.hasFinalQueries || this.isLoading || !this.isDestinationCurrentOrg
    );
  }
  ...
  get formattedSuccessReport() {
    return (this.successReportLines || []).join("\n");
  }
    ...
    try {
      ...
      if (parsed && parsed.objectName) {
        ...
        if (match) {
          ...
          this.dependencyTree = undefined;
          this.debug(
            "Selected object set from SOQL (matched):",
            this.selectedObject
          );
        } else {
  ...
  async handleTestConnection() {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        this.password = ""; // Clear password from memory after generating session ID
  ...
  async handleTestConnectionDestination() {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        this.destPassword = ""; // Clear password from memory after generating session ID
      } else {
    ...
    try {
      ...
      for (const row of sourceRows) {
        ...
        try {
          ...
          if (rows.length > 0) {
            matchedRows.push({ source: row, dest: rows[0] });
            matchedCount += 1;
          } else {
            unmatchedRows.push(row);
          }
      ...
      if (Array.isArray(results)) {
        for (const r of results) {
          if (r && r.success) {
            ...
            report.successes.push({ oldId: r.oldId, newId: r.newId });
          } else {
            report.errorCount += 1;
            report.errors.push({
              oldId: (r && r.oldId) || "",
  ...
  async handleStartImport() {
    ...
    if (!this.isDestinationCurrentOrg) {
      this.error =
        "Start Import currently supports Destination = Use Current Org";
      return;
    }
    ...
    try {
      ...
      const getTargetMap = (obj) => {
        if (!idRemap.has(obj)) idRemap.set(obj, new Map());
        return idRemap.get(obj);
      };
      ...
      const getRefTargetsByField = (objectName) => {
        ...
        if (list && list.forEach) {
          list.forEach((e) => {
            if (e && e.fieldName && e.target) map.set(e.fieldName, e.target);
          });
        }
        return map;
      };
      ...
      for (const objectName of objectsInReverse) {
        ...
        for (const def of defs) {
              ...
              if (targetObj) {
                ...
                if (mapForTarget) {
                  if (typeof value === "string" && mapForTarget.has(value)) {
                    ...
                    value = value.map((v) =>
                      typeof v === "string" && mapForTarget.has(v)
                        ? mapForTarget.get(v)
                        : v
                    );
                  }
          ...
          const insertResults = await insertRecordsCurrent({
            objectName,
            records
          });
